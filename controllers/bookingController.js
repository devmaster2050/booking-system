...
const addBooking = async (req, res) => {
    const { name, surname, email, phone, tour_id, tour_date, additional_travelers, special_requirements } = req.body;
    ...
    db.get(query, [tour_id], async (err, tour) => {
        ...
        const { tour_name, time_slots, start_date, end_date, indefinite_availability } = tour;
                    ...
                    if (slot.day_of_week) {
                        ...
                        while (current <= end) {
                            if (current.getDay() === day) {
                                availableDates.push(`${current.toISOString().split('T')[0]} ${slot.time}`);
                            }
        ...
        try {
            ...
            db.run(
                insertQuery,
                [name, surname, email, phone, tour_id, tour_date, additional_travelers, special_requirements, userId],
                async function (err) {
                    if (err) {
                    ...
                    try {
                        ...
                        await sendConfirmationSMS(phone, name, bookingId, tour_date, tour_name);
                        res.status(201).json({ message: 'Booking saved successfully!', bookingId });
                    } catch (smsError) {
                        console.error(`SMS Error: ${smsError.message}`);
                        return res.status(400).json({ message: 'Failed to send SMS. Invalid phone number or service issue.' });
                    }
                }
            );
        } catch (error) {
